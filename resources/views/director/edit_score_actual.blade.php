...
@section('content')
    <section>
        <form class="flex flex-col gap-5" action="{{ route('admin.score_actual.update', [$data->id, $data->user_id]) }}" method="post" id="form-edit-penilaian">
            @csrf
            @method('PUT')
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <div class="flex gap-1">
                        <a href="{{ route('admin.score_actual') }}" class="text-sm text-gray-500">Nilai Aktual</a>
                        ...
                        <p class="text-sm text-gray-900">Edit Penilaian - {{ $data->user->name }}</p>
                    </div>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('admin.score_actual') }}" class="text-[#B96D40] text-center border border-[#B96D40] w-32 hover:bg-[#B96D40] hover:text-white font-medium rounded-lg text-sm p-2.5">
                        Batal
                    </a>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div class="flex flex-col gap-5 bg-white w-full py-4 px-6 rounded-lg shadow-md">
                    ...
                    <div>
                        ...
                        <select id="name" name="employee_id" disabled class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#B9752E] focus:border-[#B9752E] block w-full p-2.5" required>
                            <option disabled selected>Pilih Karyawan</option>
                            @foreach ($employees as $item)
                                <option value="{{ $item->id }}" {{ $item->id == $data->user_id ? 'selected' : '' }}>{{ $item->position }} - {{ $item->role == 'division_head' ? 'Kepala Divisi' : 'Karyawan' }} - {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="flex flex-col gap-5 bg-white w-full py-4 px-6 rounded-lg shadow-md">
                    ...
                    <div class="grid grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="number" name="tahun" value="{{ $data->tahun }}" id="tahun" placeholder="Contoh: 2025" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#B9752E] focus:border-[#B9752E] block w-full p-2.5" required />
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    @foreach ($aspek as $item)
                        <div>
                            ...
                            @foreach ($data->scores as $score)
                                @if ($score->aspect_id == $item->id)
                                    <input type="number" name="scores[{{ $item->id }}]" id="{{ $item->id }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#B9752E] focus:border-[#B9752E] block w-full p-2.5" value="{{ $score->score_real }}" required />
                                @endif
                            @endforeach
                        </div>
                    @endforeach
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow-sm">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to edit this employee's score?</h3>
                        <button data-modal-hide="modal-konfirmasi" onclick="event.preventDefault(); document.getElementById('form-edit-penilaian').submit();" type="button" class="text-white bg-blue-600 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                            Yes, I'm sure
                        </button>
